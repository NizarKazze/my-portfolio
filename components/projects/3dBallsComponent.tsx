...
import { TextureLoader, InstancedBufferAttribute } from "three";
...
class X {
  ...
  #initRenderer() {
    ...
    if (!this.canvas || !(this.canvas instanceof HTMLCanvasElement)) {
      throw new Error('WebGLRenderer: canvas is null or invalid');
    }
...
class Y extends MeshPhysicalMaterial {
  ...
  constructor(params: any) {
    ...
    this.onBeforeCompile = (shader) => {
      ...
      shader.uniforms.uTextures = this.uniforms.uTextures;
      ...
      shader.fragmentShader = `
        uniform sampler2D uTextures[${MAX}];
        varying float vTextureIndex;
      ` + shader.fragmentShader;
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        `
          ...
          else texColor = texture2D(uTextures[7], vUv);
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    (this.material as Y).uniforms.uTextures.value = [];

    this.loadTextures(config.images || []);

    this.setColors(config.colors);
  }

  async loadTextures(images: string[]) {
    if (!images || images.length === 0) return;

    const loader = new TextureLoader();

    this.textures = await Promise.all(
      images.map((url) => loader.loadAsync(url))
    );
    ...
    const mat = this.material as Y;
    mat.uniforms.uTextures.value = this.textures;
    mat.needsUpdate = true;

    this.assignTextureIndices(images.length);
  }

  assignTextureIndices(imageCount: number) {
    const count = this.config.count;

    this.textureIndex = new Float32Array(count);

    const perImage = Math.floor(count / imageCount);
    ...
    for (let img = 0; img < imageCount; img++) {
      ...
      for (let j = 0; j < amount; j++) {
        this.textureIndex[i++] = img;
      }
    }

    this.geometry.setAttribute(
      "textureIndex",
      new InstancedBufferAttribute(this.textureIndex, 1)
    );
  }
  ...
  setColors(colors: number[]) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      const c1 = colorObjects[i];
      const c2 = colorObjects[Math.min(i + 1, colors.length - 1)];

      const color = new Color().copy(c1).lerp(c2, alpha);
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      spheresInstanceRef.current = ballpit;
    };
